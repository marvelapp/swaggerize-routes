        ...
        make: function (parameter, consumes) {
            ...
            return {
                ...
                validate: function validateParameter(data, callback) {
                    coerce && data && !isEqual(data, {}) && data.hasOwnProperty(parameter.name) && (data[parameter.name] = coerce(data[parameter.name]));
                    ...
                    var tmpData = {};
                    if (!isEqual(data, {})) {
                      tmpData[parameter.name] = data;
                    }

                    var ajvValid = ajvValidate(tmpData);
                    ...
                    if (!ajvValid) {
                        ...
                        var i;
                        var error;
                        for (i = 0; i < Math.min(10, ajvValidate.errors.length); i++) {
                            try {
                                error = ajvValidate.errors[i];
                                if (error.schemaPath) {
